...
///
/// ``Client`` is the main object that you interact with to communicate with the API, and is responsible for making the underlying HTTP requests for each API operation and handling the resulting responses. It implements the ``SpeakeasyAPI`` protocol to support the available API operations.
///
/// ### API calls
///
/// Each API endpoint (or *operation*) is exposed as a method. For instance, the 'deleteApiEndpoint' operation is exposed as:
///
/// ```swift
...
public final class Client {
    ...
    internal typealias ResponseHandler<ResponseObject> = (_ response: SpeakeasyResponse) throws -> ResponseObject
        ...
        return try await withCheckedThrowingContinuation { continuation in
            makeRequest(with: server, configureRequest: configureRequest) { result in
                do {
                    ...
                    switch error {
                    case .internal:
                        continuation.resume(throwing: SpeakeasyError.internalError(error: nil))
                    case .missingDefaultServer(let serverType):
                        continuation.resume(throwing: SpeakeasyError.failedToConstructRequestURL(.missingDefaultServer(serverType: serverType)))
                    case .invalidServerIndex(let serverType, let index):
                        continuation.resume(throwing: SpeakeasyError.failedToConstructRequestURL(.invalidServerIndex(serverType: serverType, index: index)))
                    case .missingServerParameterSubstitutionKey(let key, let serverString):
                        continuation.resume(throwing: SpeakeasyError.failedToSerializeRequestParameters(.missingServerParameterSubstitutionKey(key, serverString: serverString)))
                    }
                    ...
                    case .invalidURL(let urlString):
                        continuation.resume(throwing: SpeakeasyError.failedToConstructRequestURL(.invalidRequestURL(string: urlString)))
                    case .missingPathParameterSubstitutionKey(let key, let path):
                        continuation.resume(throwing: SpeakeasyError.failedToSerializeRequestParameters(.missingPathParameterSubstitutionKey(key, path: path)))
                    }
                } catch let error as SerializationError {
                    switch error {
                    case .failedToSerializeData:
                        continuation.resume(throwing: SpeakeasyError.failedToSerializeRequestParameters(.failedToSerializeData))
                    case .missingRequiredRequestBody:
                        continuation.resume(throwing: SpeakeasyError.failedToConstructRequest(.missingRequiredRequestBody))
                    case .invalidSerializationParameter(let type, let format):
                        continuation.resume(throwing: SpeakeasyError.failedToSerializeRequestParameters(.invalidSerializationParameter(type: type, format: format)))
                    }
                } catch let error as ResponseHandlerError {
                    switch error {
                    case .failedToDecodeJSON(let error):
                        continuation.resume(throwing: SpeakeasyError.failedToHandleResponse(.failedToDeserializeJSON(error)))
                    }
                } catch let error as SpeakeasyError {
                    continuation.resume(throwing: error)
                } catch {
                    continuation.resume(throwing: SpeakeasyError.internalError(error: error))
                }
            }
        }
    }
    ...
    internal func makeRequest(
        ...
        completion: @escaping (Result<SpeakeasyResponse, Swift.Error>) -> Void
    ) {
        do {
    ...
    private func baseURL(serverOverride: Server? = nil) throws -> URL {
        ...
        guard let url = URL(string: server.urlString) else {
            throw SpeakeasyError.failedToConstructRequestURL(.invalidServerURL(string: server.urlString))
        }
        return url
    }

    private func makeDataRequest(with urlRequest: URLRequest, completion: @escaping (Result<SpeakeasyResponse, Swift.Error>) -> Void) {
        let task = session.dataTask(with: urlRequest) { data, response, error in
            if let error {
                completion(.failure(SpeakeasyError.failedToMakeNetworkRequest(error: error)))
            } else if let httpResponse = response as? HTTPURLResponse {
                completion(.success(SpeakeasyResponse(httpResponse: httpResponse, data: data)))
            } else {
                completion(.failure(SpeakeasyError.internalError(error: nil)))
            }
        }
        task.resume()
    }
