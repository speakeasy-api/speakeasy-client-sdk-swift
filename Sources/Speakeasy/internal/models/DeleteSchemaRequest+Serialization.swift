...
extension Operations.DeleteSchemaRequest: Serializable {
    func serialize(with format: SerializableFormat) throws -> String {
        switch format {
        case .path:
            return try serializePathParameterSerializable(self, with: format)
        case .query, .header, .multipart, .form:
            throw SerializationError.invalidSerializationParameter(type: "Operations.DeleteSchemaRequest", format: format.formatDescription)
        }
    }
}

extension Operations.DeleteSchemaRequest: PathParameterSerializable {
